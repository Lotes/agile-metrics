...
namespace SVGViewer
{
	public class Shape : ClipArtElement
	{
		...
		protected virtual void Parse(SVG svg, string name, string value)
		{
			...
			{
				GetStroke(svg).LineCap = (SVGViewer.Stroke.eLineCap)Enum.Parse(typeof(SVGViewer.Stroke.eLineCap), value);
				return;
			}
			if (name == SVGTags.sStrokeLinejoin)
			{
				GetStroke(svg).LineJoin= (SVGViewer.Stroke.eLineJoin)Enum.Parse(typeof(SVGViewer.Stroke.eLineJoin), value);
				return;
			}
